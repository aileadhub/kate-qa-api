...
// Known QA column IDs per project (discovered 2026-06-15; not all projects have one)
const KNOWN_QA_GROUP_IDS = {
  '0tCS8B8WoLruD': 'vgGU5kMbOlYiHS', // potomac-movers: "QA"
  'NlPuRYaeSjPh_': 'oZEBndKukhkBj5', // ellis-moving: "QA"
  'Z7QKdCHVkc2m':  'AM2SwqaPe5Pq_!', // amp-movers: "QA"
  'EcwsSC!_Ao':    '70SmwO7G9j',     // fair-play-movers: "QA"
};

export async function updateTaskStatus(taskId, projectId, statusName) {
  if (statusName === 'Completed') {
    // Nifty API requires the string "true" (not boolean) to mark a task complete
    await niftyFetch('PUT', `/tasks/${taskId}`, { completed: 'true' });
    return { task_id: taskId, status: 'Completed' };
  }
  if (statusName === 'QA') {
    let groupId = KNOWN_QA_GROUP_IDS[projectId] ?? null;
    if (!groupId) groupId = await findTaskGroupByName(projectId, 'qa');
    if (!groupId) throw new Error(`Task group "QA" not found in project ${projectId}`);
    await niftyFetch('PUT', `/tasks/${taskId}`, { task_group_id: groupId });
    return { task_id: taskId, status: 'QA', task_group_id: groupId };
  }
...
export async function createTask(clientSlug, title, description) {
  ...
  // Column naming varies by project; some have empty "Ready for Dev" columns (no tasks →
  // invisible to task-based name lookup). Always prefer the known ID when available.
  const KNOWN_DEV_GROUP_IDS = {
    '4m35tboROc':      '67GZiMqmapuDP', // helix: "Ready for Dev"
    utyYIrvBdn7rt:     'd3YUrUHvbDZDZP', // dependable-movers: "Ready for Dev"
    'NlPuRYaeSjPh_':   'ZuhudLV0195jN!', // ellis-moving: "Ready for Dev"
    'Kk__BrrIfw':      'Tb3IGn_iU9',     // absolute-movers: "Ready for Dev"
    'EcwsSC!_Ao':      'Nrexbk3h7c',    // fair-play-movers: "Ready for Dev"
  };
  let groupId = KNOWN_DEV_GROUP_IDS[projectId] ?? null;
  if (!groupId) groupId = await findTaskGroupByName(projectId, 'ready for dev');
  if (!groupId) throw new Error(`"Ready for Dev" column not found in project for ${clientSlug}`);
