...
export async function postComment(taskId, text) {
  const data = await niftyFetch('POST', '/messages', { task_id: taskId, type: 'text', text });
...
export async function createTask(clientSlug, title, description) {
  ...
  // Column naming varies by project. Try common dev column names, then fall back
  // to "ready for qa" for boards (like helix) that have no dedicated dev column.
  ...
  if (!groupId) groupId = await findTaskGroupByName(projectId, 'ready for qa');
  if (!groupId) throw new Error(`Dev column not found in project for ${clientSlug} — checked "Ready for Dev", "In Dev", "Dev", "Ready for QA"`);
  ...
  try {
    await niftyFetch('POST', '/messages', { task_id: taskId, type: 'text', text: '@Pol Riba please review' });
  } catch (_) {
